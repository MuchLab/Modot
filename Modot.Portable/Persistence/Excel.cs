...
public static class Excel {
    ...
    public static List<T> FromExcel<T>(string path, string sheetName) where T : class, new(){
        List<T> rows = new List<T>();
        using (var stream = File.OpenRead(path.Replace(ResourcePathPrefix, "")))
        {
            rows = stream.Query<T>(excelType: ExcelType.XLSX, sheetName: sheetName).ToList();
        }
        return rows;
    }

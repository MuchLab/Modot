...
public class ConditionalDecorator<T> : Decorator<T>, IConditional<T>
{
	...
	public override TaskStatus Update(T context, float delta)
	{
		...
		_conditionalStatus = ExecuteConditional(context, delta);
	...
	internal TaskStatus ExecuteConditional(T context, float delta, bool forceUpdate = false)
	{
		if (forceUpdate || _shouldReevaluate || _conditionalStatus == TaskStatus.Invalid)
			_conditionalStatus = _conditional.Update(context, delta);

...
public abstract class Composite<T> : Behavior<T>
{
	...
	protected void UpdateLowerPriorityAbortConditional(T context, float delta, TaskStatus statusCheck)
	{
		...
		for (var i = 0; i < _currentChildIndex; i++)
		{
			...
			if (composite != null && composite.AbortType.Has(AbortTypes.LowerPriority))
			{
				...
				var status = UpdateConditionalNode(context, delta, child);
	...
	/// <param name="context">Context.</param>
	/// <param name="statusCheck">Status check.</param>
	protected void UpdateSelfAbortConditional(T context, float delta, TaskStatus statusCheck)
	{
	...
	TaskStatus UpdateConditionalNode(T context, float delta, Behavior<T> node)
	{
		if (node is ConditionalDecorator<T>)
			return (node as ConditionalDecorator<T>).ExecuteConditional(context, delta, true);
		else
